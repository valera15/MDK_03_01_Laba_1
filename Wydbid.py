...
# CompanyLogin - авторизация компании 
# LoadingScreen - загрузка
from UI.Login import CompanyLogin, LoadingScreen 
from UI.WydbidUI import WydbidUIMain # основной интерфейс
from sqlalchemy.orm import sessionmaker
from UI.WydbidUI.Prefabs import Settings # настройки
# SettingsLogic - загрузка и применение настроек 
# WydbidUIMainLogic - перезагрузка клиентов
...
# Функция обновления (подтягивает обновления проекта)
def finalUpdate():
...
# Cброс приложения при перезагрузке
def reset():
...
# Закрывает сессию базы данных и завершает приложение
def close():
    sessionmaker.close_all()
    app.exit(0)

# Гарантия существования структуры папок данных 
def buildLocation():
...
# Визуализация и работа с данными (запуск)
